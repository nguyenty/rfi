...
When FDR is controlled at $5\%$ in both dataset, there are 444 common DE genes between two RFI lines in which only 1 common DE gene with log2fc at least 1.
% <<result ='hide'>>=
...
When FDR is controlled at $20\%$ for DE Analysis of G8 data, and FDR is controlled at $5\%$ for DE Analysis of single end count G9 data, paired end count G9 data, there are 17 common DE genes between RFI lines
<<>>=
