...
list_cov_out1_cvm <-  data.frame(Date=as.Date(character()),
                                 File=character(), 
                                 User=character(), 
...
for(i in 3){ # i <- 1
  ...
  repeat{
    ...
    list_cov_out1_cvm <- rbind(list_cov_out1_cvm, res)
    if (cov_del ==1) break
...
res.outnew_cvm <- list(best.model.est = best.model.est[[nrep]], 
                fdr.est = fdr.est[[nrep]],
                rt.est = rt.est[[nrep ]])
save(res.outnew_cvm, file = paste0("res.outnew_cvm_", nrep, ".RData"))
 write.csv(list_cov_out1_cvm, file = paste0("list_cov_out1_cvm_",nrep,  ".csv"), row.names = FALSE)
