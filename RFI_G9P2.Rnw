...
\item Possible CBC (Complete Blood Count) data:  neutrophils, lymphocytes, monocytes, eosinophils, and basophils. The CBC covariates are in model in the form of log transformation.
\end{itemize}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
...
\item Models with metadata covariates and without CBC covariates: The number of genes analyzed is 15058. Those are genes with average counts greater than one and for which there are at least four samples with non-zero counts. 

\item Models with metadata covariates and without CBC covariates: The number of genes analyzed is 12222. Those are genes with average counts greater than 8 and for which there are at least four samples with non-zero counts. 
\end{itemize}
}
...
\begin{frame}[fragile]
\frametitle{Backward Model Selection (Without CBC Covariates)}
\begin{figure}[h!]
    \centering
...
\begin{frame}[fragile]
\frametitle{AIC Model Selection (Without CBC covariates)}
\begin{itemize}
...
\section{Results}
\begin{frame}[fragile]
\frametitle{Results of Model 11 (Without CBC covariates)}
\begin{itemize}
\item 
...
\begin{frame}[fragile]
\frametitle{Backward Model Selection (With CBC Covariates)}
\begin{figure}[h!]
    \centering
    %\includegraphics[width=\textwidth,height=0.8\textheight,keepaspectratio]{/home/ntyet/research/RFI-newdata/analysis/rfi/Plot06_26_2014_1.pdf}
