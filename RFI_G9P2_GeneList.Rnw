...
lf120 <- degene20[abs(log2(apply(counts[degene20, Line ==1]+1, 1, mean)/apply(counts[degene20, Line ==2]+1, 1, mean))) >=1]
 scount[rownames(counts[lf120,]), 1]# List of  Line DE Genes with log2(FC) >=1 when FDR is controled at 0.20
...
<<results='asis'>>=
# Summary table
out <- data.frame(FDR =c(0.05, 0.10, 0.15,0.20), degene = degene, lf1 = lf1)
...
de_gene_ex <- subset(kept_gene_ex, line_qvalue <=0.20)
dim(de_gene_ex)

de_log2fc1_gene_ex <- subset(de_gene_ex, abs(log2fc) >=1)

write.table(kept_gene_ex, file = "kept_gene_single_end.txt" ,
            sep = "\t", 
            row.names = FALSE)
write.table(de_gene_ex, file = "DE_gene_single_end_qvalue20.txt" ,
            sep = "\t", 
            row.names = FALSE)
write.table(de_log2fc1_gene_ex, file = "log2fc1_DE_gene_single_end_qvalue05.txt",
            sep = "\t", 
            row.names = FALSE )
...
norm_count_single <- sweep(kept_gene[,-1], 2,STATS =  offset, FUN="/")
dim(norm_count_single)
summary(norm_count_single)
norm_count_single_ex <- cbind(GeneID =kept_gene_ex[,1], 
                              norm_count_single, 
                              kept_gene_ex[,33:(dim(kept_gene_ex)[2])])

head(norm_count_single_ex)
write.table(norm_count_single_ex, file = "norm_count_single.txt" ,
            sep = "\t", 
            row.names = FALSE)
@
