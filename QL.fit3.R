...
if(Model=="NegBin"&!NBdisp %in% c("trend","common", "tagwise")&length(NBdisp)!=nrow(counts)) stop("Unidentified NegBin Dispersion: NBdisp must be set as 'trend' or 'common' to estimate negative binomial dispersion from data using GLM edgeR (McCarthy et al., 2012),
...
d<-calcNormFactors(d)
d$samples$norm.factors <- calcNormFactors(counts)
#?calcNormFactors
### Try tagwise estimation of dispersion parameter
# if(NBdisp=="tagwise"){
#   d <- estimateGLMCommonDisp(d, design,...)
#   nb.disp <- estimateGLMTagwiseDisp(d, design,...)$tagwise.dispersion
# }
