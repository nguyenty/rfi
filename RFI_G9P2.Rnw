...
\section{Results}
\begin{frame}[fragile]
\frametitle{Single End Read Count Data-Results of Model 7}
\begin{itemize}
\item 
...
lf105 <- which(abs(log2(apply(counts[degene05, Line ==1]+1, 1, mean)/apply(counts[degene05, Line ==2]+1, 1, mean))) >=1)

lf110 <- which(abs(log2(apply(counts[degene10, Line ==1]+1, 1, mean)/apply(counts[degene10, Line ==2]+1, 1, mean))) >=1)

lf115 <- which(abs(log2(apply(counts[degene15, Line ==1]+1, 1, mean)/apply(counts[degene15, Line ==2]+1, 1, mean))) >=1)


lf120 <- degene20[abs(log2(apply(counts[degene20, Line ==1]+1, 1, mean)/apply(counts[degene20, Line ==2]+1, 1, mean))) >=1]
...
colnames(out) <- c("FDR", "DEGs", "$\\mbox{log2(FC)}\\geq 1$")
...
\begin{frame}[fragile]
\frametitle{Paired End Read Count Data-Result of Model 7}
<<results='hide'>>=
...
lf120 <- degene20[abs(log2(apply(counts[degene20, Line ==1]+1, 1, mean)/apply(counts[degene20, Line ==2]+1, 1, mean))) >=1]
 scount[rownames(counts[lf120,]), 1]# List of  Line DE Genes with log2(FC) >=1 when FDR is controled at 0.15
...
When FDR is controlled at $5\%$, then there are 444 common DE genes between two RFI lines.
<<>>=
scount <- read.table("single end uniquely mapped reads count table for Yet.txt", 
                     header = T)
...
load("Model7_result.RData")
degene05_s <- which(result$Q.values[[3]][,"Line"] <=0.05)
id_de_s <- kept_gene$GeneID[degene05_s]
#length(id_de_s)

pcount <- read.table("paired end uniquely mapped reads count table.txt", 
                     header = T)

## List of Genes used to find DE Genes
pcount <- pcount[-c(which(pcount[,1] %in%"ENSSSCG00000007978"),
                    which(pcount[,1] %in%"ENSSSCG00000014725")),]

kept_gene_p <- pcount[rowSums(pcount[,-1]>0)>3&
                           rowMeans(pcount[,-1])>8 ,]
#dim(kept_gene_p)
#dim(pcount)
#kept_gene$GeneID
...
load("Model7_resultdat2.RData")
degene05_p <- which(result$Q.values[[3]][,"Line"] <=0.05)
id_de_p <- kept_gene_p$GeneID[degene05_p]
#length(id_de_p)
length(intersect(id_de_p, id_de_s))
...
When FDR is controlled at $20\%$ for DE Analysis of G8 data, and FDR is controlled at $5\%$ for DE Analysis of single end count G9 data, there are 23 common DE genes between RFI lines
<<>>=
gene_list <- c("ENSSSCG00000000810", "ENSSSCG00000009051", "ENSSSCG00000027314", 
"ENSSSCG00000006457", "ENSSSCG00000002792", "ENSSSCG00000007517", 
"ENSSSCG00000024791", "ENSSSCG00000021802", "ENSSSCG00000000625", 
"ENSSSCG00000008614", "ENSSSCG00000012141", "ENSSSCG00000016426", 
"ENSSSCG00000013293", "ENSSSCG00000007656", "ENSSSCG00000021646", 
"ENSSSCG00000008193", "ENSSSCG00000001511", "ENSSSCG00000015766", 
"ENSSSCG00000009829", "ENSSSCG00000006309", "ENSSSCG00000006378", 
"ENSSSCG00000006912", "ENSSSCG00000003716")
gene_list
@
and among those there are 3 genes
with log2fc $\geq 1$. 
<<>>=
commongenel2fc1 <- c("ENSSSCG00000000625", "ENSSSCG00000015766", "ENSSSCG00000006912")
commongenel2fc1
@
\end{frame}
\end{document}