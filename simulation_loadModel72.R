...
for ( i in 2:8){ # i <- 8
  ee_coef <- sort(result$Q.values[[3]][,i], index.return = T)$ix[(dim(coef_beta)[1]-result$m0[3,i-1]+1):dim(coef_beta)[1]]
  coef_beta[ee_coef,i] <- 0
  if (i ==9) coef_beta[ee_coef, c(i+1, i+2)] <- 0
}
